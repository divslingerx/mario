...
import { Sides, Align } from '../Entity';
...
export function connectEntity(pipeEntity: Entity, travellerEntity: Entity) {
    const pipeTrait = pipeEntity.getTrait<Pipe>(Pipe);
    if (!pipeTrait) {
        //  should and error be thrown here?
        return;
    }
...
export  class Pipe extends Trait {
    ...
    public travellers= new Map<Entity, ReturnType<typeof createTravellerState> >();
 

    addTraveller(pipe: Entity, traveller: Entity) {
        const pipeTraveller = traveller.getTrait<PipeTraveller>(PipeTraveller);
        if(!pipeTraveller) {
            return;
        }
    ...
    collides(pipe: Entity, traveller: Entity) {
        ...
        if (!traveller.getTrait(PipeTraveller) || this.travellers.has(traveller)) {
            return;
        }

      

        if (traveller.getTrait<PipeTraveller>(PipeTraveller)?.direction.equals(this.direction)) {
