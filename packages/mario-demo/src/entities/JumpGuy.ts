...
export class JumpGuy extends Entity {
...
export async function loadJumpGuy(audioContext: AudioContext) {
  const [jumpguySprites, audioBoard] = await Promise.all([
    loadSpriteSheet('jumpguy'),
    loadAudioBoard('jumpguy', audioContext),
  ])

  const runAnimation = jumpguySprites.getAnimation('run')

  return function createJumpGuy() {
    return new JumpGuy(jumpguySprites, audioBoard, runAnimation)
  }
}
