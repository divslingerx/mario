...
export const loadKoopaGreen = async () => {
  const sprites = await loadSpriteSheet("koopa-green");

  return () => new Koopa(sprites);
};

export const loadKoopaBlue = async () => {
  const sprites = await loadSpriteSheet("koopa-blue");
